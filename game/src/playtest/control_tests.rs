use crate::engine::playtest::advance_playtest_control_for_game_state;
...
fn profile_with_down_input(input: &str) -> PlaytestControlProfile {
...
fn named_profile_resolution_uses_canonical_builtin_constant() {
    let resolved = resolve_control_profile(&PlaytestControlProfileRef::Named(
...
fn named_profile_resolution_prefers_inline_then_file_then_builtin() {
    ...
    let resolved_inline =
        resolve_control_profile(&PlaytestControlProfileRef::Inline(inline_profile.clone()))
            .unwrap();
    let resolved_file = resolve_control_profile(&PlaytestControlProfileRef::Named(
        BUILTIN_PROFILE_GROUNDED_WALK_RIGHT.to_string(),
    ))
    .unwrap();
    ...
    let resolved_builtin = resolve_control_profile(&PlaytestControlProfileRef::Named(
        BUILTIN_PROFILE_GROUNDED_WALK_RIGHT.to_string(),
    ))
    .unwrap();
...
fn playtest_control_waits_for_playing_state_before_advancing() {
    ...
    install_active_playtest_control_timeline(ActiveControlTimeline::new(
        resolve_control_profile(&PlaytestControlProfileRef::Named(
            BUILTIN_PROFILE_GROUNDED_WALK_RIGHT.to_string(),
        ))
        .unwrap(),
    ));
...
#[test]
fn playtest_control_does_not_advance_outside_playtest_mode() {
    let _guard = ControlStateGuard::new();
    reset_engine_session_state();
...
fn chaos_expansion_is_deterministic_for_same_seed() {
    let config = PlaytestChaosConfig { seed: 7 };
...
fn expanded_chaos_profile_can_be_replayed_as_normal_profile() {
    let expanded = expand_chaos_profile(&PlaytestChaosConfig { seed: 11 });
