use super::session::PlaytestRuntimeSession;
use super::snapshot::{
...
fn extras_with_player_velocity_x(value: f32) -> ron::Value {
    ron::Value::Map(ron::Map::from_iter(vec![(
        ron::Value::String("player_velocity_x".to_string()),
        ron::Value::Number(value.into()),
    )]))
}
...
fn runtime_control_request_updates_manifest_and_snapshot_state() {
    ...
    .unwrap();
    let mut session = PlaytestRuntimeSession::unattached(
        "session-1".to_string(),
...
fn valid_runtime_request_is_consumed_and_applied() {
    ...
    let request = PlaytestSnapshotRequest {
        extras: extras_with_player_velocity_x(2.5),
    };
    let mut app = PlaytestApp::new(PlaytestLaunchArgs {
        payload_path: "payload.ron".to_string(),
    });
    app.session.attach_transport(transport.clone());
...
fn manifest_mirrors_latest_accepted_snapshot_request() {
    ...
    app.session.initialize_manifest("payload.ron".to_string());
...
fn malformed_runtime_control_request_file_is_not_deleted() {
    ...
    assert!(PlaytestRuntimeSession::consume_control_runtime_request(&transport).is_none());
    assert!(transport.control_request_path().exists());
