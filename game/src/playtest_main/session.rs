...
use engine_core::playtest::{
    PlaytestActiveControl, PlaytestControlRequest, PlaytestSessionManifest, PlaytestSessionRole,
    PlaytestSessionState, PlaytestSessionTransport, PlaytestSnapshot, PlaytestSnapshotRequest,
};
...
pub(crate) struct PlaytestRuntimeSession {
    session_id: String,
...
impl PlaytestRuntimeSession {
    ...
    pub(crate) fn write_snapshot(&self, snapshot: &PlaytestSnapshot) {
        ...
        if let Err(error) = transport.write_snapshot(snapshot) {
            onscreen_error!("Failed to write snapshot: {error}");
        }
    }
    ...
    pub(crate) fn initialize_manifest(&mut self, payload_path: String) {
        ...
        if let Err(error) = transport.write_manifest(&manifest) {
            onscreen_error!("Failed to write session manifest: {error}");
            return;
        }
    ...
    pub(crate) fn update_manifest_state(&mut self, state: PlaytestSessionState) {
        ...
        if let Err(error) = transport.write_manifest(&manifest) {
            onscreen_error!("Failed to update session manifest: {error}");
            return;
        }
    ...
    pub(crate) fn apply_runtime_control_request(
        &mut self,
        request: PlaytestControlRequest,
    ...
    fn update_manifest_active_control(&mut self, request: Option<PlaytestControlRequest>) {
        let (Some(transport), Some(mut manifest)) =
            (self.transport.as_ref(), self.manifest.clone())
        else {
            return;
...
pub(crate) fn session_dir_for_launch(launch_args: &PlaytestLaunchArgs) -> PathBuf {
    ...
    let session_dir_name = payload_path
        ...
        .map(|stem| format!("{stem}{}", playtest_artifacts::SESSION_DIR_SUFFIX))
        .unwrap_or_else(|| "playtest_session".to_string());
    payload_path
        .parent()
