...
/// Path to the file-defined agent playtest control profiles folder for the current game.
pub fn playtest_control_profiles_folder() -> PathBuf {
    resources_folder_current().join(agents::PLAYTEST_CONTROL_PROFILES_FOLDER)
}
    ...
    fn playtest_control_profiles_folder_lives_under_resources() {
        ...
        assert_eq!(
            ...
            Some(std::ffi::OsStr::new(
                crate::constants::agents::PLAYTEST_CONTROL_PROFILES_FOLDER,
            ))
        );
    }
}
