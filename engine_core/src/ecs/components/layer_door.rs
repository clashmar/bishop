...
/// Validates one authored `LayerDoor` against room-layer requirements.
///
/// Returns only actionable editor warnings. Internal invariant failures are
/// treated as non-user-facing and only guarded in debug builds.
