...
fn run_single_pilot(session_dir: &Path, profile: &str, wait: bool) -> Result<PilotOutput, String> {
    ...
    let control_request_path = session_dir.join(playtest_artifacts::CONTROL_REQUEST_FILENAME);
...
fn read_snapshot_summary(session_dir: &Path) -> Result<SessionSnapshotSummary, String> {
    let manifest_path = session_dir.join(playtest_artifacts::SESSION_FILENAME);
    let snapshot_path = session_dir.join(playtest_artifacts::SNAPSHOT_FILENAME);
...
fn session_dir_for_payload(payload_path: &Path) -> PathBuf {
    ...
    payload_path
        ...
        .unwrap_or_else(|| PathBuf::from("playtest_session"))
}
...
fn payload_path_for_session_dir(session_dir: &Path) -> Option<PathBuf> {
    Some(session_dir.parent()?.join(format!(
        ...
        session_dir
            .file_name()?
            .to_string_lossy()
            .trim_end_matches(playtest_artifacts::SESSION_DIR_SUFFIX)
    )))
}
