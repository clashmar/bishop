...
impl CleanupFixture {
    fn new() -> Self {
        ...
        fs::write(session_dir.join(agents::MANIFEST_FILENAME), "session").unwrap();
        fs::write(session_dir.join(agents::SNAPSHOT_FILENAME), "snapshot").unwrap();
        fs::write(
            session_dir.join(format!(".{}.tmp", agents::SNAPSHOT_FILENAME)),
            "tmp",
        )
        .unwrap();
...
fn session_dir_can_be_derived_from_game_playtest_command_line() {
    let session_dir = PathBuf::from("/tmp/playtest_example_agent");
...
fn read_snapshot_summary_exposes_direct_camera_fields() {
    ...
    let snapshot = PlaytestSnapshot {
        ...
        payload: Some(payload!(
            camera_target: (15.0_f32, 18.0_f32),
            camera_zoom: (1.25_f32, 1.25_f32),
    ...
    fs::write(
        fixture.session_dir.join(agents::MANIFEST_FILENAME),
        ron::to_string(&manifest).unwrap(),
    )
    .unwrap();
    fs::write(
        fixture.session_dir.join(agents::SNAPSHOT_FILENAME),
